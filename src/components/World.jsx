...
export default function World() {
  ...
  return (
    <section className="w-full pt-24 sm:pt-28 pb-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col justify-center min-h-screen overflow-x-hidden">
      {/* Header Section */}
      <div className="text-center mb-6 max-w-full">
        <h2 className="text-2xl sm:text-4xl lg:text-5xl font-avenir font-extrabold tracking-tight text-slate-900 uppercase">
          DISCOVER THE WORLD OF <span className="text-[#ff6b00]">Drag Drive Simulator</span>
        </h2>
        <p className="font-avenir text-slate-600 text-xs sm:text-base max-w-2xl mx-auto mt-1 leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center w-full max-w-full">
        ...
        <div className="lg:col-span-7 relative group rounded-3xl overflow-hidden shadow-xl border border-slate-200/80 bg-slate-900 text-white min-h-[350px] sm:min-h-[400px] flex flex-col justify-between transition-all duration-300 w-full max-w-full">
          ...
          <div className="relative z-10 p-4 sm:p-5 flex justify-between items-start">
            <span
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-avenir font-bold uppercase tracking-wider text-white shadow-md backdrop-blur-md"
              style={{ backgroundColor: activeLoc.badgeColor }}
            >
              <Sparkles className="w-3 h-3" />
              {activeLoc.badge}
            </span>
            <span className="font-avenir font-bold text-white/40 text-2xl sm:text-3xl">0{activeIdx + 1}</span>
          </div>
          ...
          <div className="relative z-10 p-4 sm:p-6 space-y-2.5 sm:space-y-3">
            <div>
              <div className="flex items-center gap-1.5 text-amber-400 font-avenir font-semibold text-xs sm:text-sm mb-0.5">
                ...
                <span className="truncate">{activeLoc.subtitle}</span>
              </div>
              <h3 className="text-2xl sm:text-4xl lg:text-5xl font-avenir font-extrabold tracking-wide text-white uppercase leading-none truncate">
                {activeLoc.name}
              </h3>
            </div>

            <p className="font-avenir text-slate-300 text-xs sm:text-sm leading-relaxed max-w-xl line-clamp-3 sm:line-clamp-none">
              {activeLoc.description}
            </p>

            {/* Quick Stats Grid */}
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5 pt-2.5 border-t border-white/15 w-full">
              {activeLoc.stats.map((st, i) => (
                <div key={i} className="bg-white/10 backdrop-blur-md p-1.5 sm:p-2 rounded-xl text-center min-w-0 overflow-hidden">
                  <span className="block text-[9px] sm:text-[10px] font-avenir text-slate-300 uppercase tracking-wider truncate">{st.label}</span>
                  <span className="block text-[10px] sm:text-xs font-avenir font-bold text-white mt-0.5 truncate">{st.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-5 space-y-2.5 w-full max-w-full min-w-0">
          ...
          {locations.map((loc, idx) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-2.5 sm:gap-3 p-2 sm:p-2.5 rounded-2xl cursor-pointer transition-all duration-200 border w-full max-w-full overflow-hidden ${
                  isActive
              ...
              >
                {/* Thumbnail image */}
                <div className="relative w-16 h-12 sm:w-20 sm:h-14 rounded-xl overflow-hidden shrink-0 shadow-sm">
                ...
                <div className="flex-1 min-w-0 overflow-hidden">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <h5 className={`font-avenir font-bold text-sm sm:text-lg uppercase leading-tight truncate ${isActive ? 'text-[#ff6b00]' : 'text-slate-900'}`}>
                      {loc.name}
                    </h5>
                    <span
                      className="px-1.5 sm:px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-avenir font-bold uppercase text-white shrink-0"
                      style={{ backgroundColor: loc.badgeColor }}
                    >
                      {loc.category}
                    </span>
                  </div>
                  <p className="font-avenir text-[10px] sm:text-[11px] text-slate-500 truncate mt-0.5">
                    {loc.subtitle}
                  </p>
                </div>
              </div>
